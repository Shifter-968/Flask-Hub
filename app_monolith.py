    ...
    if request.method == "POST":
        ...
        if action in {"post_charge", "post_payment", "post_adjustment"}:
            ...
            try:
                ...
                if txn_type in {"charge", "payment"} and account.get("parent_user_id"):
                    ...
                    if txn_type == "charge":
                        message = f"A new charge of E {amount:.2f} was added for {account.get('student_name')}. New balance: E {new_balance:.2f}."
                    else:
                        message = f"A payment of E {amount:.2f} was received for {account.get('student_name')}. New balance: E {new_balance:.2f}."
                    notify_user(
                ...
                flash(f"{txn_type.title()} transaction recorded.", "success")
            except Exception as e:
                if _finance_table_missing_error(e):
                    flash(
                        "Finance tables are missing. Run schema_finance_ops.sql in Supabase first.", "error")
        ...
        if action == "send_reminder":
            ...
            result = notify_user(
                ...
                notification_type="finance",
                priority="high" if balance > 0 else "normal",
                send_email=True,
                send_sms=balance > 0,
                meta={
                    "event": "fee_reminder",
            ...
            try:
                supabase.table("fee_reminder_runs").insert({
                    "school_id": school_id,
                    "account_id": account_id,
                    "channel": "email_sms" if balance > 0 else "email",
